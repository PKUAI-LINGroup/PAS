...
def train_supervised(model, train_loader, test_loader, args, writer, iteration):
    ...
    EPOCHS = args.supervised_epoch + iteration // 2
    best_epoch = 0
    best_acc = 0.0
    model.train()
    for epoch in range(EPOCHS):
        ...
        if test_acc > best_acc:
            ...
            best_epoch = epoch
        writer.add_scalar(f'{iteration}_labeled_train_loss', running_loss/len(train_loader), epoch)
        writer.add_scalar(f'{iteration}_labeled_dev_acc', test_acc, epoch)
        writer.add_scalar(f'{iteration}_labeled_dev_loss', test_loss, epoch)
        writer.add_scalar(f'{iteration}_labeled_best_epoch', best_epoch, epoch)
        model.train()
...
def main(args):
    ...
    Iteration = 0
    np.save(args.save_dir+f'iteration{Iteration}_train_data', train_data)
    np.save(args.save_dir + f'iteration{Iteration}_rest_data', rest_data)
    ...
    while len(rest_data)>0:
        ...
        model.load_state_dict(torch.load(args.checkpoints_dir+f'iteration{Iteration}_supervised_weights'))
        pseudo_labeled_data, rest_data = predict(model, rest_loader)
        train_data = train_data + pseudo_labeled_data
        print(len(train_data), len(rest_data))
        Iteration += 1
        np.save(args.save_dir + f'iteration{Iteration}_train_data', train_data)
        np.save(args.save_dir + f'iteration{Iteration}_rest_data', rest_data)
        trainset = Parallel(in_list=True, L=train_data)
        restset = Parallel(in_list=True, L=rest_data)
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.lbsz, shuffle=True)
        if len(restset) > 0:
            rest_loader = torch.utils.data.DataLoader(restset, batch_size=args.ubsz, shuffle=True)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/home1/wangyh/imdb/')
    parser.add_argument('--supervised_epoch', type=int, default=5)
    parser.add_argument('--checkpoints_dir', type=str, default='/home1/wangyh/imdb/self/checkpoints/')
    parser.add_argument('--save_dir', type=str, default='/home1/wangyh/imdb/self/iter_data/')
    parser.add_argument('--lbsz', type=int, default=16)
    parser.add_argument('--ubsz', type=int, default=8)
